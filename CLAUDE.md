...
- **The gate is green before every commit** — `npm run verify` (type-check, knip, build). There are
  no git hooks: nothing runs it for you. Never leave the tree broken.
...
The gate is `npm run verify` — `ci:ts` (type-check), `ci:knip` (dead code), then `npm run build`. No
git hooks run it for you; CI runs it on every PR and again on `main`. Dev server: `npm run dev`.
Full table: [development.md](docs/standards/development.md#full-command-reference).
